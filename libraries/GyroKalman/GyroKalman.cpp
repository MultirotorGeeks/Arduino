 ...
 * Function:	GetEstCov()
 * Scope:		Public
 * Arguments:	uint8_t	i	- Desired element of the estimate covariance matrix. Values:
 ...
 */
_lAccum GyroKalman :: GetEstCov ( uint8_t i ) {
...
void GyroKalman :: Predict ( long StepTime_us, _lAccum rateActuation_radps2 ) {
  ...
  if ( angle_pred > PILK )
    angle_pred -= TWOPILK;                                              // if angle is greater than pi, subtract 2pi to normalize
  else if ( angle_pred < -PILK )
    angle_pred += TWOPILK;                                              // else if angle is less than -pi, add 2pi to normalize
...
void GyroKalman :: Update_with_NoData ( void ) {
  ...
  if ( angle_est > PILK )
    angle_est -= TWOPILK;                                               // if angle is greater than pi, subtract 2pi to normalize
  else if ( angle_est < -PILK )
    angle_est += TWOPILK;                                               // else if angle is less than -pi, add 2pi to normalize

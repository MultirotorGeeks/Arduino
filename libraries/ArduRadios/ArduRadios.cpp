 ...
 */
uint8_t ArduRadio::groundInitFilt(uint8_t filtCoeffs[4]){
	uint8_t rtn = RADIO_RTN_SUCCESS;	// initial return value indicating success.  Will be replaced if failure occurs
	uint16_t lastRadioVals[4];			// holds the latest radio reading taken while performing average calculation
	int16_t meanRadioVals[4] = {0};	    // holds the mean value of radio readings so far times 16 (for better precision)
	int16_t deltaRadioVals[4] = {0};	// holds the difference between latest and mean readings times 16 (for better precision)
 ...
 */
uint8_t ArduRadio::airInitFilt(uint8_t filtCoeffs[4]){
	uint8_t rtn = RADIO_RTN_SUCCESS;	// initial return value indicating success.  Will be replaced if failure occurs
 ...
 */
uint8_t ArduRadio::read_radio(uint16_t radiovals[4]){
	...
	sprintf(str,"Roll: %4d    Pitch: %4d    Throttle: %4d    Yaw: %4d\n",
			radio_in[CH_ROLL],
			radio_in[CH_PITCH],
 ...
 */
void ArduRadio::Timer_0_1_ISR(void){
 ...
 */
void ArduRadio::Timer_2_3_ISR(void){
	uint16_t cnt = TCNT1;			// read the timer value
 ...
 */
ISR(PCINT2_vect) {
	ArduRadios.Timer_0_1_ISR();			// call the timer 0&1 interrupt service routine
	return;
 ...
 */
ISR(PCINT0_vect) {
	ArduRadios.Timer_2_3_ISR();			// call the timer 2&3 interrupt service routine
	return;
}	// end of ISR(PCINT2_vect)
